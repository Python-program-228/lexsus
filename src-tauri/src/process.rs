//! Process registry (Agent Runtime, Phase 1).
//!
//! Every child process the app spawns — PTY shells running `run_command`,
//! MCP server processes — is registered here with its pid, kind and owning
//! task. The registry answers three questions the runtime cannot answer
//! otherwise:
//!
//!   * what is running right now (`list`),
//!   * who owns it (`ProcessEntry.task_id`),
//!   * how to stop it safely (`kill` — SIGTERM first, SIGKILL only after a
//!     grace period, and to the whole process *group* so a shell's
//!     children don't survive it).
//!
...
impl ProcessKind {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            ProcessKind::McpServer => "mcp_server",
        }
    }
}
...
/// The app-wide registry. Global because `bridge::execute` (which spawns
/// the PTY) has no access to Tauri state; registration must not depend on
...
impl ProcessRegistry {
    ...
    pub fn register(
        ...
        // Persistence to the `processes` table happens at the call sites
        // (lib.rs), which own the SQLite connection — the registry itself
        // stays storage-agnostic and unit-testable.
        self.processes.lock().unwrap().insert(id, entry);
        id
    }

    /// Remove a process from the registry (normal exit). Safe to call
    /// twice.
    ...
    /// Registry ids of every process owned by a task.
    pub fn by_task(&self, task_id: &str) -> Vec<u64> {
        self.processes
            .lock()
            .unwrap()
            .values()
    ...
    /// Stop everything owned by a task (used by task cancellation).
    pub fn kill_task(&self, task_id: &str, grace: Duration) -> usize {
        let ids = self.by_task(task_id);
        let n = ids.len();
        for id in ids {
            self.kill(id, grace);
        }
...
fn kill_tree(pid: u32, grace: Duration) {
    ...
    // PTY children are session leaders, MCP children are spawned in their
    // own process group — in both cases the process group id equals the
    // child pid, so `kill(-pid, …)` reaches the whole tree. If the group
    // signal fails (ESRCH), fall back to the single pid.
    unsafe {
...
fn kill_tree(pid: u32, _grace: Duration) {
    ...
    // Windows: no process groups; open and terminate the single process.
...
mod tests {
    ...
    fn by_task_filters() {
        let reg = ProcessRegistry::new();
        reg.register(999_998, ProcessKind::Command, "a", Some("task_1".into()));
        reg.register(999_997, ProcessKind::McpServer, "b", Some("task_2".into()));
        assert_eq!(reg.by_task("task_1").len(), 1);
        assert_eq!(reg.by_task("nope").len(), 0);
    }
