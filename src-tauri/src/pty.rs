...
pub enum PtyChunk {
    ...
    Data(Vec<u8>),
    /// Coalesced notice: this many chunks were dropped because the channel    /// was full. Emitted when capacity frees — never per-drop, so a busy    /// UI cannot cause an overflow-notification feedback loop.
    Dropped(u64),
}
